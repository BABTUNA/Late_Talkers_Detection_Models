...
class FusionModel(LightningModule):
    ...
    def forward(self, joint_input_ids, audio_input_ids, text_input_ids, mask=None, **kwargs):
        ...
        fuse1 = torch.stack((joint_embed, audio_embed, text_embed), dim=1) 
    ...
    def preprocess_dataframe(self):
        ...
        df = self.args.data_updated  #pd.read_json(self.data_path)
        ...
        self.train_data = TensorDataset(
            ...
            torch.tensor(np.array(df_train['text_feature_scaled'].tolist()), dtype=torch.float),
        ...
        self.test_data = TensorDataset(
             ...
             torch.tensor(np.array(df_test['text_feature_scaled'].tolist()), dtype=torch.float),
    ...
    def test_epoch_end(self, outputs):
        ...
        pred_df.to_csv(
            f'ckf_att_{self.args.index}_{self.args.random_seed}_{self.args.heads}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_test_pred.csv', index=False)
        ...
        df_result.to_csv(
            f'ckf_att_{self.args.index}_{self.args.random_seed}_{self.args.heads}_{self.args.epochs}_{self.args.hidden_size}_{self.args.batch_size}_test.csv')
...
def main(args,config):
    ...
    df_data = pd.read_json('/home/benjaminbarrera-altuna/Desktop/LLE/dataset/full_data.json')
    ...
    sens = 0
    i = 0
    for train_idxs, test_idxs in group_kfold.split(df_data['audio_feature_scaled'], df_data['label'], groups):#for i, (train_idxs, test_idxs) in enumerate(kf.split(df_data, df_data['name'])):
    ...
    name = f"results_{config['random_seed']}_{config['heads']}_{config['epochs']}_{config['hidden_size']}_{config['batch_size']}.csv"
...
if __name__ == '__main__': 
    ...
    parser.add_argument("--text_feats", type=int, default=300)
