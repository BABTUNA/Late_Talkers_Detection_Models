...
class FusionModel(LightningModule):
    ...
    def forward(self, joint_input_ids, audio_input_ids, text_input_ids, mask=None, **kwargs):
        ...
        a2j, _ = self.mha_j(audio_embed, joint_embed, joint_embed)
        a2j = self.norm_t(joint_embed + a2j) 

        t2j, _ = self.mha_j(text_embed, joint_embed, joint_embed)
        t2j = self.norm_t(joint_embed + t2j) 
        
        # Audio cross

        j2a, _ = self.mha_j(joint_embed, audio_embed, audio_embed)
        j2a = self.norm_t(audio_embed + j2a) 


        t2a, _ = self.mha_j(text_embed, audio_embed, audio_embed)
        t2a = self.norm_t(audio_embed + t2a) 


        # Text cross

        j2t, _ = self.mha_j(joint_embed, text_embed, text_embed)
        j2t = self.norm_t(text_embed + j2t) 

        a2t, _ = self.mha_j(audio_embed, text_embed, text_embed)
        a2t = self.norm_t(text_embed + a2t) 

  
        fuse1 = torch.stack((t2a, t2j, a2t, a2j, j2a, j2t), dim=1) 
